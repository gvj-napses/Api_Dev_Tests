...
const user = {
  name: "user",
  create: (user) => [new CreateUserQuery(user.id,user.name,user.email,user.mobile)],
  build: () => entity("user"),
  delete: (user) => [new DeleteEntityById(user.id, "users")],
};

const aadhar = {
  name: "aadhar",
  create: (aadhar) => [new CreateAadharQuery(aadhar.user.id,aadhar.id,aadhar.name,aadhar.aadharnumber)],
  build: () => entity("aadhar"),
  delete: (aadhar) => [new DeleteEntityById(aadhar.id, "aadharcarddetails")],
...
const address = {
  name: "address",
  create: (address) => [new CreateAddressQuery(address.id,address.name,address.street,address.city,address.country,address.user.id)],
  build: () => entity("address"),
  delete: (address) => [new DeleteEntityById(address.id, "addresses")],
  dependency: [[(address) => address.user, user]],
};

const role = {
  name: "role",
  create: (role) => [new assignRoleQuery(role.id,role.user.id,role.id)],
  build: () => entity("role"),
  delete: (role) => [new DeleteEntityById(role.id, "roles")],
  dependency: [[(role) => role.user, user]],
};
